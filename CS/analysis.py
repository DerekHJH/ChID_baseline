...
def main():
    ...
    config = AutoConfig.from_pretrained('./1w')
    tokenizer = AutoTokenizer.from_pretrained('./1w')
    model = BertForChID.from_pretrained('./1w')
    ...
    if training_args.do_eval:
        ...
        pred_eval = trainer.predict(test_dataset=eval_dataset)

        pred_test = trainer.predict(test_dataset=test_dataset)
