...
# import wandb
# wandb.init(project="ChID", entity="whohow", name="chid-bert-base-chinese")
...
def main():
    ...
    # Uncomment the following 6 lines, when you just want to experiment with few data.
    # raw_datasets = DatasetDict(
    #     {
    #         'train': raw_datasets['train'].shuffle().select(range(100)),
    #         'validation': raw_datasets['validation'].shuffle().select(range(100)),
    #         'test': raw_datasets['test'].shuffle().select(range(100))
    #     }
    # )
        ...
        for i, sentence in enumerate(first_sentences):
            if len(sentence) > 465:
                if sentence.find(tokenizer.mask_token*4) > len(sentence) // 2:
                    first_sentences[i] = sentence[-465:]
                else:
                    first_sentences[i] = sentence[:465]
            
            assert len(candidates[i]) == 7
            first_sentences[i] =  tokenizer.sep_token.join(candidates[i]) \
                + tokenizer.sep_token + first_sentences[i]
